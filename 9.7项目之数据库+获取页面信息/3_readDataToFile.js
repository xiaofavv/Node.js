...
var out=fs.createWriteStream("./data.txt");//数据写出流  我们将读到的数据存到data.txt中
...
connection.connect(function(err){
    if(err){
        ...
        result.on("fields",function(fields){//读取返回结果集中每一列的列信息
            var str="";
            fields.forEach(function(fields){//循环所有的列
                str+=fields.name+"\t";
            });
            out.write(str+"\r\n");
        });
        result.on("result",function(row){//每读取一行记录，触发一次
                connection.pause();//每读到一条数据，先暂停读取后面后面的数据
                out.write(row.s_sid+"\t"+row.s_sname+"\t"+row.s_cid+"\t"+row.s_sex+"\t"+row.s_age+"\t"+row.s_pwd+"\t"+row.s_tel+"\t"+row.c_cid+"\t"+row.c_cname+"\t"+row.s_status+"\r\n","utf8",function(err){
                    if(err){
                        console.info("数据写入文件失败...");
                        process.exit();//退出程序
                    }else{
                        //如果没有错误。继续取下一行数据
                        connection.resume();
